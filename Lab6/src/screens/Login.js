...
const Login = ({ onLoginSuccess }) => { // Accept onLoginSuccess as a prop
    const navigation = useNavigation(); // Get the navigation object
    const [phone, setPhone] = useState(''); // State for phone number
    const [password, setPassword] = useState(''); // State for password
    const [error, setError] = useState(null);
    ...
    const handleLogin = async () => {
        ...
        try {
            ...
            Alert.alert('Successful');
            onLoginSuccess(); // Call the onLoginSuccess function to update the state in the parent component
            navigation.navigate('TabNavigator'); // Navigate to TabNavigator on successful login
        } catch (err) {
            setError(err);
            console.error('Error:', err);
            Alert.alert('Login failed', 'Please check your credentials and try again.'); // Show error alert
